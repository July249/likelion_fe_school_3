...
- 원시타입(primitive types) (7가지!) : number, string, boolean, null, undefined, symbol(ES6 추가, 변경 불가능한 유일한 값)
...
- Array(배열) : object
  ...
  - 메서드 :
    ...
    - map - 새로운 배열을 반환
      ...
      ```
    - filter - 반환값이 true인 것만 배열에 담아 반환
    - push / pop - mutable
    - slice - immutable
    - splice - mutable
    - reduce - immutable
  ...
  let array = Array.from({ length: 10 }, (v, idx) => idx);
